...
import storage
import scoring
from scoring import compute_score, interpret_score, recommendations_for, predict_progression
...
class DiseaseResult(BaseModel):
    ...
    forecast: Dict # New field for ML prediction
    explanation: str = "No explanation" # XAI
    timestamp: datetime
...
# --------------------------------------------------
# STORAGE (Now handled by storage.py via SQLite)
# --------------------------------------------------
...
def analyze(patient_id: str, disease: str, data: AnswerSet):
    ...
    previous_score = storage.get_last_score(patient_id, disease)
    
    # 3. Interpret Status
    status, trend, interpretation = interpret_score(current_score, previous_score)
    
    # 4. Save to DB
    storage.save_score(patient_id, disease, current_score)
    
    # 5. ML Forecasting (Retrain on new history)
    full_history = storage.get_history(patient_id, disease)
    forecast = predict_progression(full_history)
    ...
    return DiseaseResult(
        ...
        recommendations=recommendations_for(disease, current_score),
        forecast=forecast,
        explanation=forecast.get("explanation", "N/A"),
        timestamp=datetime.utcnow()
    )

@app.get("/explain-trend/{patient_id}/{disease}")
def explain_trend_only(patient_id: str, disease: str):
    """
    Direct endpoint for Module 5. 
    Returns only the Forecast + SHAP Explanation based on historical data.
    """
    full_history = storage.get_history(patient_id, disease)
    if not full_history:
...
def home():
    return {
        "message": "Module 4 Disease Progress Tracker Running",
        "note": "This system supports monitoring, not diagnosis"
    }
